  ...
  <div class="wrapper col-sm-12">
    <div class="container col-sm-12"><!--Spisak svih kategorija-->
      <div class="jumbotron  offset-xl-2 col-xl-8">
        <fieldset>
          <legend>Spisak svih kategorija:</legend>
          <ul class="list-group">
            <li class="list-group-item justify-content-between">
              <span class="badge badge-default badge-pill">Naziv kategorije:</span>
              <span class="badge badge-default badge-pill" style="float:right;">Broj tema:</span>
            </li>
          </ul>
            <ul class="list-group" v-for='kategorija in listaKategorijaC'>
              <li class="list-group-item justify-content-between">
                {{ kategorija.ktg_naziv}}
                <span class="badge badge-pill badge-info" style="float:right;" v-if="kategorija.broj">{{ kategorija.broj }}</span>
                <span class="badge badge-pill badge-info" style="float:right;" v-else="kategorija.broj">0</span>
              </li>
            </ul>
          </fieldset>
      </div><!--kraj spiska svih kategorija-->
      <div class="jumbotron offset-xl-2 col-xl-8"><!--Dodavanje nove kategorije-->
        <fieldset>
          <legend>Dodaj novu kategoriju:</legend>
        <div class="col-sm-12">
          <label for="exampleInputEmail1">Naziv kategorije</label>
          <input type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Naziv" v-model="naziv">
          <small id="emailHelp" class="form-text text-muted"></small>
        </div>
        <div class="form-group">
          <label for="exampleSelect1">Izaberi ikonicu za kategoriju</label>
          <select class="form-control" id="exampleSelect1" v-model="ikonica">
        ...
        <button v-on:click="dodajKategoriju" type="submit" class="btn btn-success" id="insertCategory">Dodaj novu kategoriju</button>
        </fieldset>
      </div><!--kraj dodavanja nove kategorije-->
      <div id="kat" class="jumbotron offset-xl-2 col-xl-8"><!--Brisanje kategorije-->
        <fieldset>
          <legend>Izaberi kategoriju za brisanje:</legend>
            <select class="custom-select" v-model="selected">
              <option value="" selected disabled>Izaberite kategoriju za brisanje</option>
              <option v-for="kategorija in listaKategorijaC" v-bind:value="kategorija.ktg_id">{{ kategorija.ktg_naziv }}</option>
            </select>
            <button v-on:click="obrisiKategoriju(selected)" type="button" name="button" class="btn btn-success" id="dugme">Potvrdi</button>
          </fieldset>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods:{
    listaKategorija: function(){
      this.$store.dispatch('listaKategorijaA_F');
    },
    dodajKategoriju: function(){
      if(this.naziv == '' || this.ikonica == ''){
      ...
      this.$store.dispatch('dodajKategorijuA_F',kat);
      this.naziv = '';
      this.ikonica = '';
      }
    },
    obrisiKategoriju: function(selected){
      var selected = this.selected;
      this.$store.dispatch('obirisKategorijuA_F',selected);
      this.selected = '';
    }
  },
  computed:{
    listaKategorijaC: function(){
      return this.$store.getters.listaKategorijaG_F;
    }
  },
  created(){
